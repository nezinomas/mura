...
<div class="card w-full shadow-xl border {{ $isMine ? 'bg-slate-50 border-slate-200 mura-grab-card' : 'bg-base-100 border-base-300' }}">
    <div class="card-body p-8">
        ...
        <div class="flex justify-between items-start pb-6 text-sm text-base-content/60">
            <div>
                @if($post->user)
        ...
        <div class="max-w-none leading-relaxed text-base-content whitespace-pre-wrap pt-2 pb-6">{!! $post->content_html !!}</div>

        @if(isset($actions))
            <div class="flex justify-end gap-4 pt-6 border-t border-base-300/50 text-sm">
                {{ $actions }}
            </div>
        @endif
    </div>
